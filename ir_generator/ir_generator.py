    ...
    for block in blocks:
                    ...
                    if d["Имя"] == name:
                        ...
                        for c_d in c_data:
                            if "Имя" in c_d and "Значение" in c_d:
                                kb_query_data.append({"term_name": c_d["Имя"],
                                                       "feature_name": None,
                    ...
                    # Остальные варианты
                        ...
                        for c in d["Характеристики"]:
                            if "Имя" in c and "Значение" in c:
                                kb_query_data.append({"term_name": d["Имя"],
                                                      "feature_name": c["Имя"],
        ...
        for d in kb_query_data:
            ...
            if "feature_path" in d and d["feature_path"] is not None:
                ...
                append_qualitative_value(   cq_successors,
                                            d["kb_feature_name"],
                                            "Характеристика",
                                            d["kb_value_name"],
                                            None,
                ...
                append_qualitative_value(successors,
                                         d["kb_term_name"],
                                         group_name,
                                         d["kb_value_name"],
                                         None,
...
def append_qualitative_value(parent_successors, name, meta, value, measure, date, time, feature_path, value_path):
...
if __name__ == "__main__":
    if len(sys.argv) != 5:
        print("Usage: ir_generator.py input.json template.json output.json fts_db_config.json")
        sys.exit(1)
