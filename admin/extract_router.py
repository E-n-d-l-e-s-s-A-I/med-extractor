...
def extract_tab_page(term_tab):
    ...
    with term_tab:
        ...
        if uploaded_file:
            ...
            """
            st.components.v1.html(html_with_scroll, height=350)
        ...
        if st.session_state.extract_processing:
            with st.spinner("Идет извлечение терминов..."):
                try:
                    ...
                    text_block = extract_chapter_texts(html_content, "extract_result.json", config)
                    ...
                    llm_requests = prompt_llm(text_block, config, text_block, "extract_result.json")
                    ...
                    with open("extract_result.json", "r", encoding="utf-8") as file:
                        ...
                        text_placeholder.text_area(
                            ...
                            value=json.dumps(st.session_state.extract_result, indent=4, ensure_ascii=False),
                            height=500,
                    ...
                    for i, _ in enumerate(llm_requests):
                        ...
                        status_text.text(
                            ...
                            (f" из {st.session_state.total_requests}" if st.session_state.total_requests > 0 else "")
                        )
                    ...
                    generate_ir("extract_result.json", "ir_generator/samples/IR_Template.json", "ir.json", "ir_generator/configs/fts_db_config.json")
